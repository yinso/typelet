...
class Type
  ...
  convert: (obj, path = '$', isExplicit = true, context = @convert) ->
...
class PrimitiveType extends Type
  ...
  isAssignableFrom: (type) -> if type == @ then true else false
...
class TypeBinder
  constructor: (@types = []) ->
    for type in @types
      if not type instanceof AnyType
        throw new Error("TypeBinder.requires_any_type")
    @binders = []
  bind: (typeVar, type) ->
    for [ tVar , tVal ] in @binders
      if typeVar == tVar and not type.equal(tVal)
        throw new Error("TypeBinder.type_already_bound: #{tVar} to #{tVal} != #{type}")
    @binders.push [ typeVar , type ]
...
class ProcedureType extends Type
  ...
  typeEnv: () -> # what do we want to return? a list of typeVars
